...
.modal-bg{position:fixed;inset:0;background:rgba(30,26,22,0.55);display:flex;align-items:center;justify-content:center;z-index:100;padding:24px}
.modal{background:white;border:1px solid var(--border2);border-radius:3px;padding:36px;max-width:400px;width:100%;position:relative}
.modal::after{content:'';position:absolute;top:0;left:20px;right:20px;height:1px;background:linear-gradient(90deg,transparent,var(--gold3),transparent)}
.modal-title{font-family:var(--serif);font-size:19px;font-weight:500;letter-spacing:0.1em;margin-bottom:6px}
.modal-sub{font-size:12px;color:var(--ink3);letter-spacing:0.05em;margin-bottom:28px;line-height:1.7}
.field{margin-bottom:18px}
.field label{display:block;font-size:10px;letter-spacing:0.18em;text-transform:uppercase;color:var(--gold);margin-bottom:8px}
.field input{width:100%;background:var(--paper);border:1px solid var(--border);border-radius:2px;padding:11px 14px;font-family:var(--sans);font-size:14px;font-weight:300;color:var(--ink);outline:none;transition:border-color .2s}
.field input:focus{border-color:var(--gold2);background:white}
.field input::placeholder{color:var(--ink3)}
.field-row{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.stripe-badge{display:flex;align-items:center;justify-content:center;gap:6px;font-size:11px;color:var(--ink3);margin-top:14px;letter-spacing:0.06em}
.modal-close{position:absolute;top:14px;right:18px;background:none;border:none;color:var(--ink3);cursor:pointer;font-size:18px}
.success-box{text-align:center;padding:20px 0}
.success-icon{font-size:36px;margin-bottom:16px}
.success-title{font-family:var(--serif);font-size:18px;margin-bottom:8px}
.success-desc{font-size:13px;color:var(--ink3);line-height:1.8;letter-spacing:0.04em}
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            {!success ? (
              <>
                ...
                <div className="modal-title">プロプランに登録</div>
                <div className="modal-sub">月額 {displayPrice.toLocaleString()}円（{yearly?'年払い':'月払い'}）<br/>いつでも解約できます。</div>
                <div className="field"><label>メールアドレス</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="your@email.com"/></div>
                <div className="field"><label>カード番号</label><input type="text" placeholder="1234 5678 9012 3456" maxLength={19}/></div>
                <div className="field-row">
                  <div className="field"><label>有効期限</label><input type="text" placeholder="MM / YY"/></div>
                  <div className="field"><label>セキュリティコード</label><input type="text" placeholder="123" maxLength={4}/></div>
                </div>
                <button className="plan-btn primary" onClick={handlePay} disabled={processing}>{processing?'処理中...':`${displayPrice.toLocaleString()}円で始める`}</button>
                <div className="stripe-badge"><span>🔒</span><span>Stripe で安全に処理されます</span></div>
              </>
            ) : (
              <div className="success-box">
                <div className="success-icon">🪷</div>
                <div className="success-title">ご登録ありがとうございます</div>
                <div className="success-desc">プロプランが有効になりました。<br/>すべての機能をご利用いただけます。</div>
                <button className="plan-btn primary" style={{marginTop:24}} onClick={()=>{setModal(false);setSuccess(false);onBack()}}>アプリへ戻る</button>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="wrap">
        ...
        <div className="plans">
          ...
          <div className="plan-card featured">
            ...
            <button className="plan-btn primary" onClick={()=>setModal(true)}>プロプランを始める</button>
          </div>
        </div>
